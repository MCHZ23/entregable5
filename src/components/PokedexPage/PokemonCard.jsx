  ...
  return (
    <article className={`pokecard ${firstType}-border`} onClick={handleClick}>
       <header className={ `pokecard__header ${firstType}-gradient` }>
           <img
             className="pokecard__image" src={pokemon?.sprites.other['official-artwork'].front_default} alt="" />
       </header>
       <section className="pokecard__body">
        ...
        { 
          pokemon?.stats.map((statInfo) => (
            <li className="pokecard__stats__url"key={statInfo.stat.url}>
              <h4 className="pokecard__stats__name">{statInfo.stat.name}</h4>
              <span className={`pokecard__stats__value ${firstType}-color` }>{statInfo.base_stat}</span>
            </li>
          ))
        }
        </ul>
